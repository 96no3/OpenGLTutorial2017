...
struct FCriWareFile
{
	IFileHandle* FileHandle;	/* ネイティブファイルハンドル	*/
...
CriFsIoError CriWareFile::Exists(const CriChar8 *Filename, CriBool *OutResult)
{
	...
	if (PlatformFile.FileExists(UTF8_TO_TCHAR(Filename))) {
		(*OutResult) = CRI_TRUE;
	} else {
		(*OutResult) = CRI_FALSE;
	}
...
CriFsIoError CriWareFile::Remove(const CriChar8 *Filename)
{
	...
	/* Remove the file */
	IPlatformFile& PlatformFile = IPlatformFile::GetPlatformPhysical();
	if (PlatformFile.DeleteFile(UTF8_TO_TCHAR(Filename))) {
	...
	CriFsFileMode Mode, CriFsFileAccess Access, CriFsFileHn *OutFile)
{
	...
	if (File == NULL) {
		UE_LOG(LogCriWareRuntime, Error, TEXT("Failed to allocate FileHandle %s."), UTF8_TO_TCHAR(Filename));
		return (CRIFS_IO_ERROR_NG);
	}
	...
	if (Access == CRIFS_FILE_ACCESS_READ) {
		File->FileHandle = PlatformFile.OpenRead(UTF8_TO_TCHAR(Filename));
	} else {
		File->FileHandle = PlatformFile.OpenWrite(UTF8_TO_TCHAR(Filename), bAppend, bAllowRead);
	}
	...
	/* ファイルサイズの取得 */
	/* Get the file size */
	File->FileSize = File->FileHandle->Size();
	...
	CriSint64 Offset, CriSint64 ReadSize, void *Buffer, CriSint64 BufferSize)
{
	...
	if (!File->FileHandle->Seek((int64)Offset)) {
		UE_LOG(LogCriWareRuntime, Warning, TEXT("Failed to Seek."));
		return (CRIFS_IO_ERROR_NG);
	}

	/* データの読み込み */
	/* Read data */
	if (!File->FileHandle->Read((uint8 *)Buffer, ReadSize)) {
		UE_LOG(LogCriWareRuntime, Warning, TEXT("Failed to Read."));
		return (CRIFS_IO_ERROR_NG);
	}
	...
	CriSint64 Offset, CriSint64 WriteSize, void *Buffer, CriSint64 BufferSize)
{
	...
	/* データの書き込み */
	/* Write data */
	if (!File->FileHandle->Write((uint8 *)Buffer, WriteSize)) {
		UE_LOG(LogCriWareRuntime, Warning, TEXT("Failed to Read."));
		return (CRIFS_IO_ERROR_NG);
	}
