...
void FCriWareStatics::HandleOnBeginFrame()
{
	...
	int32 bAdx2Profile = Value.Split(TEXT(" "), &lString, &rString) ? FCString::Atoi(*lString): FCString::Atoi(*Value);
	if (bAdx2Profile == 1) {
		Value = rString;
...
void FCriWareStatics::DrawDebugMessages(FString SortType, FString OrderType) {
	...
	{
		const int32 key = -1;
		GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::Green, TEXT("Adx2 Active Sounds:"), false);
		...
		GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::Green, TEXT("  (Second argument: SortType: id, distance, name, time)"), false);
		GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::Green, TEXT("  (Third argument: OrderType: ascending, descending)"), false);
		GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::Green, TEXT("AtomComponentID SoundCueName Distance"), false);

		int32 TotalSoundWavesNum = 0;
		for (int32 SoundIndex = 0; SoundIndex < Sorted_Items.Num(); ++SoundIndex)
		{
			if (Sorted_Items[SoundIndex].AtomComponentID > 0) { /* ID：１〜 */
				{
					const FString TheString = FString::Printf(TEXT("%4i. %s %6.2f"), Sorted_Items[SoundIndex].AtomComponentID, *Sorted_Items[SoundIndex].AtomCueName, Sorted_Items[SoundIndex].DistanceFromListener);
					GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::White, *TheString, false);
				}

				// Get the active sound waves.
				{
					const FString TheString = *FString::Printf(TEXT("    %4i. Location:(%s) PlayingTime:%6.2f PlayerState:%s"), Sorted_Items[SoundIndex].AtomComponentID, *Sorted_Items[SoundIndex].AtomComponentLocation.ToString(), Sorted_Items[SoundIndex].PlayingTime, *Sorted_Items[SoundIndex].PlayerState);
					GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::White, *TheString, false);
				}
			}
		}
		GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::Green, *FString::Printf(TEXT("Total sounds: %i"), Sorted_Items.Num()), false);
		GEngine->AddOnScreenDebugMessage(key, 0.0f, FColor::Green, *FString::Printf(TEXT("Listener position: %s"), *UCriWareInitializer::GetListeningPoint().ToString()), false);
	}
}
