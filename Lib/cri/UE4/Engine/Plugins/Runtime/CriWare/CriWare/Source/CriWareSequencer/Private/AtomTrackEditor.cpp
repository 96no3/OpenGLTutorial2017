...
void FAtomSection::BeginSlipSection()
{
	...
	InitialStartTimeDuringResize   = FFrameNumber(AtomSection->HasStartFrame() ? AtomSection->GetInclusiveStartFrame() : 0) / AtomSection->GetTypedOuter<UMovieScene>()->GetTickResolution();
}

void FAtomSection::SlipSection(double SlipTime)
{
	UMovieSceneAtomSection* AtomSection = Cast<UMovieSceneAtomSection>(&Section);
	...
	AtomSection->SetStartOffset(StartOffset);
