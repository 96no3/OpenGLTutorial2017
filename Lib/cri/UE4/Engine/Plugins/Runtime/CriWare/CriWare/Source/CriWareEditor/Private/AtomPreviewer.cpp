...
void FAtomPreviewer::Tick(float DeltaTime)
{
	...
	if (PlayerController) {
		...
		/* リスナの位置をカメラのビューポートと連動させる */
		UCriWareInitializer::SetListenerLocation(ListenerLocation);
		UCriWareInitializer::SetListenerRotation(ListenerRotation);
	}
}
...
void FAtomPreviewer::OnPreviewEnd()
{
	...
	/* ルート化されたコンポーネントの破棄 */
	for (UAtomComponent* AtomComponent : UAtomComponent::AtomRootedComponentArray) {
		AtomComponent->DestroyComponent();
	}

	/* カテゴリ情報のリセット */
	int32 NumCategories = criAtomExAcf_GetNumCategories();
	for (int32 i = 0; i < NumCategories; i++) {
		CriAtomExCategoryInfo CategoryInfo;
		criAtomExAcf_GetCategoryInfo((CriUint16)i, &CategoryInfo);
		criAtomExCategory_SetVolumeById(CategoryInfo.id, 1.0f);
		criAtomExCategory_PauseById(CategoryInfo.id, CRI_FALSE);
	}
